...
func _ready():
	...
	print("DEBUG MODE: play_level_four")
	...
	GLOBALVAR.HP = 100
	GLOBALVAR.four = 0
	GLOBALVAR.four_star_one = false
	GLOBALVAR.four_star_two = false
	GLOBALVAR.four_star_three = false
	GLOBALVAR.Bfood = 100
	GLOBALVAR.Bwater = 100
	GLOBALVAR.makan_ayam = 0
	GLOBALVAR.minum_ayam = 0
	GLOBALVAR.suhu_dingin = 0
	GLOBALVAR.suhu_panas = 0
	GLOBALVAR.minum_obat = 0
...
func _process(delta):
	...
	if GLOBALVAR.makan_ayam >= 3 && GLOBALVAR.minum_ayam >= 3:
		GLOBALVAR.four_star_one = true
	if GLOBALVAR.suhu_dingin >= 2 && GLOBALVAR.suhu_panas >= 2:
		GLOBALVAR.four_star_two = true
	if GLOBALVAR.minum_obat == 1:
		GLOBALVAR.four_star_three = true
	
	if GLOBALVAR.four_star_one == true:
		stars.play("one")
		GLOBALVAR.a_four = 1
		GLOBALVAR.four = 1
		
	if GLOBALVAR.four_star_two == true && GLOBALVAR.four_star_one == true:
		stars.play("two")
		GLOBALVAR.a_four = 2
		GLOBALVAR.four = 2
		
	if GLOBALVAR.four_star_three == true && GLOBALVAR.four_star_two == true && GLOBALVAR.four_star_one == true:
		stars.play("three")
		GLOBALVAR.a_four = 3
		GLOBALVAR.four = 3
		GLOBALVAR.start = 2
...
func mulai(value):
	if value == 1:
		randomize()
		print("Question 4 Dimulai !")
		hptimer.start()
		one_timer.start()
		timer_cold.start(random(8))
		timer_hot.start(random(8))
	if value == 0:
		over_t.start()
...
func over(value):
	if value == 1:
		...
		if GLOBALVAR.a_four == 1:
			over_panel.play("one")
		if GLOBALVAR.a_four == 2:
			over_panel.play("two")
		if GLOBALVAR.a_four == 3:
